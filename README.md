...
**Версия базового тома:** v1.0 — проверено 2026-05-15. Историю изменений см. в [CHANGELOG.md](CHANGELOG.md). У прикладного тома есть отдельный журнал изменений в [book2/CHANGELOG.md](book2/CHANGELOG.md). Лицензия: текст — CC BY-SA 4.0, код — MIT (см. [LICENSE](LICENSE)).

Учебник состоит из двух томов:

- [book/](book/) — базовый том: полный SDD-цикл на учебном проекте AgentClinic.
- [book2/](book2/) — прикладной том: перенос того же цикла в production-сценарии, где нужны Spec CI, трибунал, метрики, бюджеты моделей, шлюз готовности (readiness gate) и откат.
...
[Production SDD для Qwen Code CLI](book2/README.md) — второй том для читателей, которые уже прошли базовый цикл. Он разбирает лабораторную модель AgentClinic-production, восстановление спецификаций из legacy-следов, диагностику дефектов спецификаций, дуэли Верификатор/Имплементор, мутационное тестирование спецификаций, Spec CI, многоагентный трибунал, ярусные бюджеты, метрики Гудхарта, интеграцию с production API, диагностические антипаттерны прикладного цикла и практический зачёт по production SDD.
