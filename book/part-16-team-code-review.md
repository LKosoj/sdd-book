...
### Нативный путь: `/review` и `qwen-code-action`

С версии 0.13.0 (анонс 27 марта 2026 года) в Qwen Code есть встроенная команда `/review`: мультиагентное ревью, где 9–10 параллельных агентов разбирают diff по логике, безопасности, производительности, стилю и тестам. Команда принимает локальный diff, номер PR или его URL (`/review 123`), а флаг `--comment` публикует находки инлайн-комментариями прямо в запросе на слияние. Для CI есть официальный GitHub Action `QwenLM/qwen-code-action` — он ставится командой `/setup-github`, разворачивает workflow `qwen-dispatch.yml`, запускается автоматически на `pull_request` и дополнительно реагирует на комментарий `@qwencoder /review`.

Нативный `/review` хорошо ловит общие проблемы кода, но не знает про `requirements.md`, `plan.md` и `validation.md` конкретной фичи — сверка diff со спецификацией специфична для SDD-процесса, и её приходится описывать отдельно. Ниже — учебный каркас именно для этой задачи, а не замена `qwen-code-action`.

Минимальный каркас — workflow в `.github/workflows/`, который срабатывает на событие `pull_request`. Описывать его лучше по шагам, раз речь о сверке со спецификацией фичи, а не об общем ревью кода:
