...
- `/review` — встроенное ревью кода: параллельные LLM-ревью-агенты плюс проверка CI-статуса перед одобрением; это не детерминированный шлюз — для жёстких проверок используйте отдельный проектный скрипт.
- `/skills` — просмотр и явный запуск навыков.
- `/memory`, `/remember`, `/forget`, `/dream` — управление памятью и `QWEN.md`, включая консолидацию (`/dream` — штатная команда; в старых релизах, где её нет, оформляйте консолидацию как пользовательскую команду).
...
| Имплементор (Implementor) — голосует | Qwen Code в режиме default/auto-edit после утверждённого плана (в актуальной документации режим default называется «Ask Permissions Mode»; алиас default в конфиге сохраняется) |
