...
Spec CI проверяет не только текстовые требования. Если агент строит план как набор атомарных действий, этот план тоже становится проверяемым артефактом. AoT здесь — паттерн «Atom of Thought»: план раскладывается на атомы с полями `id`, `kind`, `dependsOn`, а корректность графа проверяется топологической сортировкой (открытый учебный прообраз — `examples/10_aot-agent` в репозитории `pguso/ai-agents-from-scratch`). Не путайте с «Atom of Thoughts» (arXiv 2502.12018) — это другая техника, декомпозиция вопроса при test-time reasoning. Формат может быть простым: `atoms[]`, где каждый атом имеет `id`, `kind`, `name`, `input` и `dependsOn`.
