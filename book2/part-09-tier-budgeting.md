...
## Базовый уровень: таблица выбора модели по задаче

Прежде чем переходить к ярусированию по фазам, риску и очереди (его и описывает остальная часть главы), полезно зафиксировать стартовую эвристику выбора модели. Она оперирует не именами моделей конкретного провайдера, а классами по мощности и стоимости: лёгкий, средний и тяжёлый. В разных проектах этим классам соответствуют разные модели — важна не марка, а соотношение «дорого/умно».

| Класс модели | Когда применять |
|---|---|
| Лёгкий | исследование и поиск файлов, простые однофайловые правки, генерация документации |
| Средний | многофайловая реализация, ревью pull-запросов — модель по умолчанию для ~90% задач кодирования |
| Тяжёлый | сложная архитектура, анализ безопасности, отладка сложных багов |

Средний класс держите как значение по умолчанию: он закрывает большинство задач кодирования. Лёгкий берите на дешёвую рутину, где ошибка легко обнаруживается и откатывается. Тяжёлый включайте не по привычке, а по правилу апгрейда:

- первая попытка на среднем классе провалилась;
- задача затрагивает пять и более файлов;
- принимается архитектурное решение;
- код критичен для безопасности.

Эта таблица — базовый уровень той же логики, что развёрнута дальше в главе. Связь прямая: `local-coder` соответствует лёгкому/среднему классу на потоковой рутине, `frontier-reviewer` — тяжёлому классу на спорных и высокорисковых решениях. Различие в том, что базовая таблица выбирает модель по типу одиночной задачи, а ярусирование (tier-budgeting) добавляет три измерения, которых в базовой эвристике нет: риск и восстановимость решения, давление очереди и фаза SDD-конвейера с собственным бюджетом токенов. То есть путь развития такой: «базовый уровень выбора по задаче → ярусирование по риску, очереди и фазе». Правило апгрейда на тяжёлый класс при этом не исчезает — оно становится одним из условий эскалации с `local-coder` на `frontier-reviewer`.
