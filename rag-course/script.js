...
<p><strong>Антипаттерн:</strong> начинать оптимизацию с embedding-модели (50ms → 30ms = 4% от total latency) вместо reranker (2100ms = 40% от total). Измеряй сначала, оптимизируй потом.</p>
  ...
  {
    id: 15,
    title: "Самооптимизация Agentic RAG: управляемый внешний цикл",
    goal: "Научиться улучшать RAG-систему через воспроизводимые эксперименты, многокритериальный отбор и утверждение изменений человеком.",
    objectives: [
      "Различать внутренний цикл ответа и внешний цикл улучшения системы",
      "Описывать изменяемую конфигурацию RAG как версионируемый SOP",
      "Сравнивать кандидатов на отложенной выборке по качеству, задержке, стоимости и надёжности",
      "Применять пороговые ограничения, фронт Парето, поэтапный выпуск и откат"
    ],
    body: `<h2>Что именно здесь «самоулучшается»</h2>
<p>Самооптимизирующийся RAG не переобучает модель после каждого ответа. Он ищет более удачную <strong>конфигурацию системы</strong>: меняет параметры поиска, промпты, маршрутизацию и набор агентов, затем проверяет кандидатов на одном и том же наборе задач.</p>

<div class="callout callout-warn"><strong>Точное название процесса:</strong> управляемый поиск конфигурации. Слово «самообучение» здесь вводит в заблуждение: веса модели не меняются, а новая версия не должна попадать в рабочую среду без независимой проверки.</div>

<h2>Два цикла одной системы</h2>
<pre><code>Внутренний цикл, выполняется для запроса:
запрос → план → поиск → проверка контекста → ответ

Внешний цикл, выполняется для версии системы:
сбои → диагностика → кандидаты SOP → офлайн-оценка
     → пороговые ограничения → фронт Парето
     → утверждение → ограниченный выпуск → наблюдение</code></pre>

<p>Внутренний цикл решает задачу пользователя. Внешний цикл изменяет правила, по которым внутренний цикл работает. Смешивать их опасно: агент, который сам переписал промпт и сразу применил его в рабочей среде, обошёл тестирование и контроль изменений.</p>

<h2>SOP как ограниченное пространство изменений</h2>
<p>SOP (Standard Operating Procedure) — версионируемая конфигурация RAG-системы. Она должна содержать только параметры, которые разрешено изменять автоматически.</p>

<pre><code>{
  "version": "support-rag-v7",
  "retrieval": {
    "k": 8,
    "hybrid": true,
    "reranker": true,
    "metadata_filter": "active_version"
  },
  "generation": {
...
<p>Пароли, адреса хранилищ, права доступа и команды записи в базу не являются «генами» для эксперимента. Кандидат может выбрать одно из заранее разрешённых значений, но не должен произвольно переписывать инфраструктуру или исполнять сгенерированный SQL.</p>

<h2>Учебный кейс: помощник службы поддержки</h2>
<p>Синтетическая база содержит инструкции к нескольким версиям продукта. Пользователи жалуются, что помощник иногда цитирует устаревшую инструкцию. Команда формирует сто проверочных вопросов:</p>
<ul>
<li><strong>60 вопросов для разработки:</strong> по ним диагностируют сбои и создают кандидатов;</li>
<li><strong>40 отложенных вопросов:</strong> их не показывают агенту, который предлагает изменения;</li>
<li><strong>три повтора:</strong> помогают увидеть разброс результатов недетерминированной системы.</li>
</ul>

<p>Эти числа приведены для примера. Важен принцип: нельзя предлагать изменение и доказывать его пользу на одних и тех же вопросах.</p>

<h2>Оценка должна быть многомерной</h2>
<p>Одна средняя «оценка качества» скрывает опасные компромиссы. Для каждого кандидата собирают отдельный вектор:</p>
<ul>
<li><strong>качество:</strong> полнота ответа, groundedness, точность цитат;</li>
<li><strong>поиск:</strong> recall@k и доля найденных документов нужной версии;</li>
<li><strong>надёжность:</strong> доля успешных прогонов и разброс между повторами;</li>
<li><strong>эксплуатация:</strong> задержка p95, число шагов, токены и стоимость;</li>
<li><strong>безопасность:</strong> нарушения правил доступа, утечки данных и неподтверждённые действия.</li>
</ul>

<p>Детерминированные проверки отвечают за формат, ссылки, права и ограничения. LLM-as-Judge подходит для смысловых критериев, но его результаты выборочно проверяют эксперт или отдельный оценщик. Модель, которая одновременно пишет, оценивает и исправляет собственный ответ, воспроизводит свои слепые зоны.</p>

<h2>Пороговые ограничения важнее красивого среднего</h2>
<p>До сравнения по Парето кандидат проходит обязательные ограничения. Например: ноль критических нарушений безопасности, все цитаты указывают на существующие документы, задержка укладывается в SLA. Провал хотя бы одного условия исключает версию, даже если её средняя оценка высока.</p>

<pre><code>for candidate in candidates:
    result = evaluate(candidate, held_out_set, repeats=3)
    if not passes_hard_gates(result):
        reject(candidate)
    else:
        eligible.append(result)

front = pareto_front(eligible)
approved = human_select(front)</code></pre>

<h2>Фронт Парето вместо магического общего балла</h2>
<p>Кандидат считается доминируемым, если другой вариант не хуже по всем показателям и строго лучше хотя бы по одному. Доминируемые версии можно отбросить без субъективных весов.</p>

<table>
<thead><tr><th>Версия</th><th>Качество ↑</th><th>p95 ↓</th><th>Стоимость ↓</th></tr></thead>
<tbody>
<tr><td>A: базовая</td><td>0,82</td><td>2,8 с</td><td>$0,010</td></tr>
<tr><td>B: reranker</td><td>0,89</td><td>3,5 с</td><td>$0,014</td></tr>
<tr><td>C: малый контекст</td><td>0,80</td><td>2,0 с</td><td>$0,007</td></tr>
<tr><td>D: дополнительный агент</td><td>0,88</td><td>5,8 с</td><td>$0,030</td></tr>
</tbody>
</table>

<p>Версия B доминирует D: она качественнее, быстрее и дешевле. A, B и C образуют фронт Парето, потому что каждая сохраняет свой компромисс. Окончательный выбор делает владелец продукта: B — если важнее качество, C — если критична цена, A — если нужен баланс.</p>

<h2>Безопасный внешний цикл</h2>
<ol>
<li><strong>Зафиксировать базовую версию:</strong> код, SOP, модели, данные и результаты.</li>
<li><strong>Сгруппировать реальные сбои:</strong> искать причины, а не просить LLM «сделать лучше».</li>
<li><strong>Создать ограниченное число кандидатов:</strong> менять один объяснимый фактор за эксперимент.</li>
<li><strong>Провести офлайн-оценку:</strong> одинаковая отложенная выборка, повторы и независимые проверки.</li>
<li><strong>Отбросить нарушения:</strong> безопасность и SLA являются ограничениями, а не предметом торга.</li>
<li><strong>Выбрать версию с фронта Парето:</strong> решение и основание фиксирует человек.</li>
<li><strong>Выпустить поэтапно:</strong> сначала малая доля трафика, затем расширение при стабильных метриках.</li>
<li><strong>Сохранить откат:</strong> предыдущие код, индекс и SOP должны восстанавливаться как единая версия.</li>
</ol>

<div class="callout callout-tip"><strong>Критерий остановки:</strong> внешний цикл имеет лимит кандидатов и бюджета. Если улучшение меньше заранее заданного практически значимого порога или два цикла подряд не дают результата, автоматический эксперимент останавливают и переходят к ручному разбору.</div>

<h2>Почему «последняя версия» не означает «лучшая»</h2>
<p>В демонстрационных проектах часто берут последний созданный SOP и называют его текущим лучшим. Это логическая ошибка. Последний кандидат может уступать базовой версии, переобучиться на наборе разработки или нарушить обязательное ограничение. Реестр экспериментов должен хранить родителя, полный вектор метрик и решение о выпуске; рабочей становится только явно утверждённая версия.</p>

<h4>🎯 Мини-кейс</h4>
<div>
<p><strong>Ситуация:</strong> помощник поддержки путает актуальную и архивную документацию. Диагност предлагает три изменения: увеличить K с 5 до 15, добавить фильтр по версии продукта или заменить генератор более крупной моделью.</p>
<p><strong>Правильный подход:</strong> создать три отдельных кандидата и проверить их на отложенных вопросах. Если фильтр по версии повышает точность источников без роста задержки, это объяснимое исправление первопричины. Увеличение K может добавить архивный шум, а более крупная модель не исправит ошибочный retrieval.</p>
<p><strong>Антипаттерн:</strong> применить все три изменения одновременно. Даже при улучшении команда не узнает, что помогло, и не сможет безопасно откатить отдельное решение.</p>
</div>`,
    flashcards: [
      { front: "Внутренний и внешний циклы", back: "Внутренний цикл отвечает на отдельный запрос. Внешний цикл сравнивает версии системы, изменяет SOP и выпускает только проверенного кандидата." },
      { front: "SOP как пространство поиска", back: "Версионируемая конфигурация с ограниченным набором изменяемых параметров: K, фильтры, reranker, промпт, модель и лимиты. Секреты и права доступа в пространство поиска не входят." },
      { front: "Зачем нужна отложенная выборка", back: "Она проверяет переносимость улучшения на примеры, которые не использовались для диагностики и создания кандидата. Без неё легко оптимизироваться под известные вопросы." },
      { front: "Пороговое ограничение", back: "Обязательное условие допуска: безопасность, корректность цитат, SLA или бюджет. Его нельзя компенсировать высоким средним качеством." },
      { front: "Доминирование по Парето", back: "Версия A доминирует B, если A не хуже B по всем целям и строго лучше хотя бы по одной. Доминируемые варианты исключают." },
      { front: "Почему одного LLM-as-Judge недостаточно", back: "Судья может унаследовать ошибки и предпочтения генератора. Нужны детерминированные проверки, калибровка на человеческих оценках и независимый контроль." },
      { front: "Поэтапный выпуск", back: "Новую версию сначала направляют на малую долю трафика, сравнивают онлайн-метрики с базовой и расширяют только при стабильном результате." },
      { front: "Критерий остановки внешнего цикла", back: "Лимит кандидатов и стоимости плюс минимально значимое улучшение. Нет результата несколько циклов — автоматический поиск прекращается." }
    ],
    quiz: [
      {
        question: "Что действительно изменяется в описанном самооптимизирующемся RAG?",
        options: [
          "Веса LLM после каждого ответа",
          "Версионируемая конфигурация SOP после сравнения кандидатов",
          "Содержимое пользовательских запросов",
          "Датасет без сохранения версии"
        ],
        correct: 1,
        explanation: "Внешний цикл ищет лучшую конфигурацию RAG: параметры retrieval, промпты, модели и лимиты. Это оптимизация системы, а не непрерывное обучение весов модели."
      },
      {
        question: "Почему нельзя создавать и оценивать кандидатов только на одном наборе вопросов?",
        options: [
          "Оценка займёт слишком мало времени",
          "Кандидаты могут переобучиться под известные примеры и не улучшить реальные запросы",
          "LLM не умеют читать одинаковые вопросы дважды",
          "Фронт Парето требует ровно два датасета"
        ],
        correct: 1,
        explanation: "Набор разработки влияет на решения диагноста. Отложенная выборка независимо проверяет, переносится ли улучшение на невидимые примеры."
      },
      {
        question: "Кандидат повысил качество, но нарушил обязательное ограничение безопасности. Что делать?",
        options: [
          "Выпустить, если прирост качества достаточно велик",
          "Усреднить безопасность с качеством",
          "Исключить кандидата до построения фронта Парето",
          "Попросить тот же LLM изменить оценку"
        ],
        correct: 2,
        explanation: "Пороговые ограничения не являются предметом компромисса. Кандидаты с нарушением безопасности, SLA или другого обязательного условия не допускаются к отбору."
      },
      {
        question: "Версия B качественнее, быстрее и дешевле версии D. Каково отношение между ними?",
        options: [
          "D доминирует B",
          "B доминирует D",
          "Обе обязательно входят во фронт Парето",
          "Сравнение невозможно без общего среднего балла"
        ],
        correct: 1,
        explanation: "B не хуже по всем трём целям и строго лучше по каждой, поэтому она доминирует D. Общий взвешенный балл для этого вывода не нужен."
      },
      {
        question: "Какой процесс выпуска новой версии наиболее безопасен?",
        options: [
          "Последний созданный SOP сразу становится рабочим",
          "LLM выбирает собственную версию и самостоятельно заменяет рабочую",
          "Отложенная оценка → ограничения → Парето → утверждение → малая доля трафика → расширение или откат",
          "Все кандидаты по очереди получают 100% трафика"
        ],
        correct: 2,
        explanation: "Безопасный выпуск разделяет эксперимент и рабочую среду, требует явного допуска, ограничивает начальный риск и сохраняет возможность отката."
      },
      {
        question: "Помощник выбирает архивные документы. Какой кандидат лучше всего проверяет предполагаемую первопричину?",
        options: [
          "Увеличить K без фильтрации",
          "Добавить фильтр по активной версии документа",
          "Увеличить temperature генератора",
          "Добавить ещё одного агента-синтезатора"
        ],
        correct: 1,
        explanation: "Ошибка относится к области поиска и метаданных. Фильтр по активной версии исправляет причину напрямую; более крупный контекст или генератор могут лишь замаскировать проблему."
      }
    ],
    sources: [
      { title: "Autonomous Agentic RAG", desc: "Исходный пример SOP, внешнего цикла и многокритериального отбора", url: "https://github.com/FareedKhan-dev/autonomous-agentic-rag", icon: "🧬" },
      { title: "LangSmith Evaluation", desc: "Офлайн- и онлайн-оценка, датасеты, повторы и сравнение экспериментов", url: "https://docs.langchain.com/langsmith/evaluation", icon: "🧪" },
      { title: "LangGraph Interrupts", desc: "Остановка графа, сохранение состояния и утверждение действий человеком", url: "https://docs.langchain.com/oss/python/langgraph/interrupts", icon: "⏸️" },
      { title: "Optuna Multi-objective Optimization", desc: "Практический пример многокритериальной оптимизации и фронта Парето", url: "https://optuna.readthedocs.io/en/stable/tutorial/20_recipes/002_multi_objective.html", icon: "📈" }
    ]
  },
  {
    id: 16,
...
// === State ===
const PROGRESS_KEY = 'rag-completed';
const PROGRESS_VERSION_KEY = 'rag-progress-version';
let currentLesson = -1;
let completedLessons = new Set(JSON.parse(localStorage.getItem(PROGRESS_KEY) || '[]'));

if (localStorage.getItem(PROGRESS_VERSION_KEY) !== '16-lessons') {
    if (completedLessons.has(14)) {
        completedLessons.delete(14);
        completedLessons.add(15);
        localStorage.setItem(PROGRESS_KEY, JSON.stringify([...completedLessons]));
    }
    localStorage.setItem(PROGRESS_VERSION_KEY, '16-lessons');
}
