...
<h2>Дисциплина параллелизма: форки, worktrees и cascade</h2>

<p>Запустить несколько инстансов Claude Code (в разных вкладках терминала) — заманчиво, но без дисциплины это превращается в хаос конфликтов и потерянного контекста. Ключ — правильно выбрать механизм изоляции под тип задачи.</p>

<h3>Форк разговора — для непересекающихся задач</h3>
<p>Форк (ответвление разговора) подходит, когда подзадачи <strong>не пересекаются по коду</strong>: вопросы о кодовой базе, исследование внешних сервисов, чтение документации. Рекомендуемый базовый сетап: основной чат держит изменения кода, форки отвечают на вопросы о текущем состоянии кодовой базы и ведут research. Используйте <code>/rename</code>, чтобы давать чатам осмысленные имена и не путаться.</p>

<h3>Git worktrees — для пересекающейся работы над кодом</h3>
<p>Когда несколько инстансов <strong>правят код, который пересекается</strong>, форка недостаточно — нужны git worktrees. Каждый worktree это отдельная рабочая директория на своей ветке, поэтому инстансы не топчут файлы друг друга и не плодят merge-конфликты на лету:</p>

<pre><code>git worktree add ../project-feature-a feature-a
git worktree add ../project-feature-b feature-b

# Каждый worktree получает свой инстанс Claude
cd ../project-feature-a &amp;&amp; claude</code></pre>

<p>Правило: как только инстансы начинают работать над пересекающимся кодом, worktrees + чёткий план на каждый из них становятся обязательными.</p>

<h3>Метод cascade — как держать вкладки под контролем</h3>
<p>Чтобы параллелизм не съел внимание, организуйте вкладки по принципу «каскада»:</p>
<ul>
  <li>Новые задачи — в новые вкладки <strong>справа</strong>.</li>
  <li>Проходите вкладки <strong>слева направо</strong>, от старых к новым.</li>
  <li>Держите <strong>максимум 3-4 активные задачи</strong> одновременно.</li>
</ul>

<div class="callout callout-tip">
  <div class="callout-title">💡 Минимально жизнеспособная параллелизация</div>
  Не задавайте произвольное число терминалов («запущу-ка 5 локально и 5 в облаке»). Добавление инстанса должно идти от реальной необходимости, а не от желания «распараллелить ради параллелизма». Цель: <strong>сколько можно сделать при минимально жизнеспособном объёме параллелизации</strong>. Каждый лишний инстанс — это контекст, который должен удерживать в голове человек.
</div>
...
<div class="key-concept">
  <strong>Мини-кейс:</strong> Для исследования «какой state-manager выбрать» инженер запустил Fan-Out: три researcher-субагента параллельно изучили Redux Toolkit, Zustand и Jotai, каждый в чистом контексте, и вернули сжатые сводки. Оркестратор синтезировал рекомендацию за один проход. Когда тот же человек попробовал реализовать профиль пользователя через Agent Teams, расход токенов вырос в разы (peer-to-peer переговоры frontend/backend/test), но и задача была связной, а не «разложимой на независимые ветки» — это показывает, где Fan-Out достаточно, а где нужна полноценная команда.
    ...
    flashcards: [
      ...
      { front: "Минимально жизнеспособная параллелизация", back: "Не задавать произвольное число терминалов. Добавлять инстанс только при реальной необходимости. Цель — сколько можно сделать при минимальном объёме параллелизации; каждый лишний инстанс человек удерживает в голове." }
    ],
    quiz: [
      {
      ...
      {
        question: "Когда форка разговора недостаточно и нужны git worktrees?",
        options: [
          "Когда задачи это только вопросы о кодовой базе",
          "Когда несколько инстансов правят пересекающийся код и рискуют конфликтами файлов",
          "Когда нужно прочитать документацию",
          "Worktrees нужны всегда вместо форков"
        ],
        correct: 1,
        explanation: "Форк подходит для непересекающихся задач (вопросы, research). Как только инстансы работают над пересекающимся кодом, нужны git worktrees: отдельная директория на своей ветке исключает merge-конфликты на лету."
      },
      {
        question: "Что предписывает метод cascade при работе с несколькими инстансами?",
        options: [
          "Открывать все вкладки сразу и работать случайно",
          "Новые задачи справа, проходить слева направо, держать максимум 3-4 активные",
          "Запускать ровно 10 инстансов",
          "Никогда не использовать больше одной вкладки"
        ],
        correct: 1,
        explanation: "Cascade: новые задачи в новые вкладки справа, обход слева направо (от старых к новым), не более 3-4 активных задач — чтобы параллелизм не съел внимание."
      },
      {
        question: "Что означает принцип «минимально жизнеспособная параллелизация»?",
        options: [
          "Всегда запускать максимум доступных терминалов",
          "Задать фиксированное число 5 локально и 5 в облаке",
          "Добавлять инстанс только при реальной необходимости, не выбирая произвольное число",
          "Использовать только один инстанс при любых условиях"
        ],
        correct: 2,
        explanation: "Не задавайте произвольное число терминалов. Каждый новый инстанс должен идти от настоящей необходимости — цель в том, сколько можно сделать при минимальном объёме параллелизации."
      }
    ],
    sources: [
...
<h2>Итеративный retrieval: передавай цель, а не только запрос</h2>

<p>Субагенты существуют, чтобы экономить контекст: они возвращают сжатую сводку вместо свалки сырых данных. Но у оркестратора есть <strong>семантический контекст, которого у субагента нет</strong>. Субагент знает буквальную формулировку запроса, но не ЦЕЛЬ, ради которой запрос задан. Из-за этого первый возврат часто формально верен, но бесполезен: субагент ответил на букву, а не на намерение.</p>

<h3>Паттерн итеративного retrieval</h3>
<ol>
  <li><strong>Оркестратор оценивает каждый возврат субагента</strong> — не принимает результат вслепую.</li>
  <li>Задаёт <strong>уточняющие вопросы</strong> перед тем, как принять ответ.</li>
  <li>Субагент возвращается к источнику, добирает недостающее и отвечает снова.</li>
  <li><strong>Цикл повторяется до достаточности</strong> — но не больше ~3 итераций, иначе теряется выгода от делегирования.</li>
</ol>

<p>Главное правило: <strong>передавай субагенту контекст цели, а не только формулировку запроса</strong>. Сравните: «Найди, где валидируется токен» (буквальный запрос) против «Мы добавляем refresh-token endpoint и должны убедиться, что старая валидация не пропустит истёкшие токены, — найди, где валидируется токен, и оцени, переживёт ли логика новый поток» (запрос + цель). Второй вариант позволяет субагенту самому отфильтровать релевантное и предупредить о подводных камнях.</p>

<div class="callout callout-tip">
  <div class="callout-title">💡 Почему максимум ~3 итерации</div>
  Каждый цикл уточнения — это дополнительные токены и задержка. Если за 3 круга субагент не дал достаточного ответа, проблема обычно не в нём, а в постановке: цель сформулирована размыто или задача не разложима. Тогда дешевле переформулировать вход, а не крутить цикл дальше.
</div>

<h2>Оркестратор с последовательными фазами</h2>

<p>Частный, но очень надёжный случай Pipeline — жёсткая цепочка фаз, где каждая фаза это отдельный агент со своим узким мандатом:</p>

<pre><code># Последовательные фазы оркестрации
Phase 1: RESEARCH   (Explore agent)         -> research-summary.md
Phase 2: PLAN       (planner agent)         -> plan.md
Phase 3: IMPLEMENT  (tdd-guide agent)       -> изменения в коде
Phase 4: REVIEW     (code-reviewer agent)   -> review-comments.md
Phase 5: VERIFY     (build-error-resolver)  -> done или возврат на нужную фазу</code></pre>

<h3>Правила, без которых цепочка ломается</h3>
<ol>
  <li><strong>Один вход — один выход:</strong> каждый агент получает РОВНО один чёткий вход и отдаёт РОВНО один чёткий выход. Никакой агент не делает «заодно ещё и ревью».</li>
  <li><strong>Выход становится входом:</strong> результат фазы — буквально вход следующей (research-summary.md кормит planner, plan.md кормит implementor).</li>
  <li><strong>Не пропускать фазы:</strong> соблазн прыгнуть от research сразу к implement — главный источник переделок. Plan — не формальность.</li>
  <li><strong>Очищать контекст между агентами:</strong> между фазами вызывайте <code>/clear</code>, чтобы следующий агент не тащил «грязный» контекст предыдущего и стартовал с чистого окна.</li>
  <li><strong>Сохранять промежуточные выходы в файлы:</strong> research-summary.md, plan.md, review-comments.md — это и точки восстановления, и способ передать выход следующей фазе, переживающий очистку контекста.</li>
</ol>

<div class="callout callout-warn">
  <div class="callout-title">⚠️ Чем это отличается от обычного Pipeline</div>
  Обычный Pipeline описывает <em>идею</em> «выход одного = вход следующего». Фазовый оркестратор добавляет дисциплину: <code>/clear</code> между агентами и обязательную материализацию выходов в файлы. Без этого «конвейер» постепенно превращается в один разбухший контекст, и фазы перестают быть изолированными.
</div>
...
<div class="key-concept">
  <strong>Мини-кейс:</strong> SaaS-команда автоматизировала разбор входящих GitHub issues. Сначала на каждый issue запускали мощного оркестратора — дорого и медленно. Переписали как <strong>Router</strong>: дешёвый классификатор на Qwen3-Coder-Next метит issue (bug/feature/question/security) и направляет нужному субагенту. Объём вырос до сотен issue в день, а счёт остался скромным благодаря бесплатному уровню — выбор паттерна (Router вместо Orchestrator-Worker) и инструмента под него оказался важнее «силы» модели.
    ...
    flashcards: [
      ...
      { front: "Фазовый оркестратор — цепочка фаз", back: "RESEARCH → PLAN → IMPLEMENT → REVIEW → VERIFY. Каждая фаза — отдельный агент со своим мандатом; выход фазы (research-summary.md, plan.md) становится входом следующей." },
      { front: "5 правил фазового оркестратора", back: "1) Один вход — один выход. 2) Выход становится входом следующей фазы. 3) Не пропускать фазы. 4) /clear между агентами. 5) Промежуточные выходы сохранять в файлы." }
    ],
    quiz: [
      {
      ...
      {
        question: "Почему первый возврат субагента часто формально верен, но бесполезен?",
        options: [
          "Субагент использует более слабую модель",
          "Субагент знает буквальный запрос, но не ЦЕЛЬ за ним, поэтому отвечает на букву, а не на намерение",
          "У субагента нет доступа к интернету",
          "Оркестратор всегда формулирует запрос с ошибкой"
        ],
        correct: 1,
        explanation: "У оркестратора есть семантический контекст цели, которого нет у субагента. Без передачи намерения субагент отвечает на формулировку, а не на реальную задачу — отсюда итеративный retrieval с уточнениями."
      },
      {
        question: "Сколько итераций уточнения рекомендуется в паттерне итеративного retrieval?",
        options: ["Ровно 1", "Максимум ~3", "Не меньше 10", "Без ограничений, пока не идеально"],
        correct: 1,
        explanation: "Цикл «оценил возврат → задал уточнение → субагент добрал у источника» повторяют до достаточности, но не более ~3 раз: дальше дешевле переформулировать вход, чем крутить цикл."
      },
      {
        question: "Какое правило фазового оркестратора предотвращает превращение конвейера в один разбухший контекст?",
        options: [
          "Запускать все фазы параллельно",
          "/clear между агентами и сохранение промежуточных выходов в файлы",
          "Использовать одну модель на все фазы",
          "Пропускать фазу REVIEW для экономии"
        ],
        correct: 1,
        explanation: "Между фазами вызывают /clear, а выходы (research-summary.md, plan.md, review-comments.md) материализуют в файлы. Это сохраняет изоляцию фаз и даёт точки восстановления, переживающие очистку контекста."
      },
      {
        question: "Что НЕ входит в правила фазового оркестратора RESEARCH→PLAN→IMPLEMENT→REVIEW→VERIFY?",
        options: [
          "Каждый агент получает один вход и даёт один выход",
          "Выход фазы становится входом следующей",
          "Можно пропускать фазу PLAN, если задача кажется простой",
          "Промежуточные выходы сохраняются в файлы"
        ],
        correct: 2,
        explanation: "Пропуск фаз (особенно PLAN) — главный источник переделок. Правила требуют не пропускать фазы: один вход/один выход, выход→вход, /clear между агентами, выходы в файлы."
      }
    ],
    sources: [
      { title: "Building Effective AI Agents (Anthropic)", url: "https://resources.anthropic.com/hubfs/Building%20Effective%20AI%20Agents-%20Architecture%20Patterns%20and%20Implementation%20Frameworks.pdf", icon: "🤖" },
...
<h2>Гигиена контекста: сколько инструментов держать активными</h2>

<p>Описания инструментов (tools, MCP-серверы) занимают место в окне ещё до того, как агент сделает первый шаг. Каждый подключённый MCP-сервер тянет схемы всех своих инструментов, и они «съедают» бюджет молча. Практическое правило гигиены: держать <strong>менее 10 инструментов/MCP активными одновременно</strong> и в сумме <strong>менее ~80 активных инструментов</strong>.</p>

<div class="callout callout-warn">
  <div class="callout-title">⚠️ Как раздутые схемы съедают окно</div>
  Иллюстративно: при перегруженном наборе инструментов номинальное окно ~200K может «схлопнуться» до ~70K реально пригодных токенов — остальное уходит под схемы tools и системную обвязку. Цифры «200K→70K» — именно <strong>иллюстрация</strong>: конкретная потеря зависит от объёма и многословности схем подключённых инструментов, а не фиксирована. Вывод один: лишние MCP стоит выгружать (lazy-load по необходимости), а не держать включёнными «на всякий случай».
</div>

<h2>Хирургическая подача контекста через системный промпт</h2>

<p>Стратегию Select можно довести до уровня <strong>режимов работы</strong>: вместо того чтобы валить весь возможный контекст в CLAUDE.md (грузится каждую сессию), контекст подают точечно через системный промпт под конкретный режим — dev, review или research:</p>

<pre><code>alias claude-dev='claude --system-prompt \"$(cat ~/.claude/contexts/dev.md)\"'
alias claude-review='claude --system-prompt \"$(cat ~/.claude/contexts/review.md)\"'
alias claude-research='claude --system-prompt \"$(cat ~/.claude/contexts/research.md)\"'</code></pre>

<p>Так в каждом режиме грузится только релевантный срез знаний, а не всё сразу. Но у этого приёма есть ещё одно следствие — <strong>иерархия авторитета</strong>.</p>

<div class="key-concept">
  <strong>Иерархия авторитета контекста:</strong> системный промпт &gt; сообщение пользователя &gt; результат инструмента. То, что подано через системный промпт, для модели весомее, чем обычное сообщение, а сообщение — весомее, чем сырой вывод tool. Поэтому критичные правила режима (что можно/нельзя, стиль, ограничения) логично класть именно в системный промпт: они переживут шум из tool-результатов и не будут перебиты случайной фразой в чате.
</div>
...
<div class="key-concept">
  <strong>Мини-кейс:</strong> Агент рефакторил сервис за ~120 шагов. Без управления контекстом к 80-му шагу он «забыл» архитектурное решение из шага 5 и начал плодить дубли. Команда добавила три приёма: оркестратор пишет ключевые решения в <code>decisions.md</code> (Write), на каждом шаге подтягивает только затронутые файлы (Select), а поиск по коду делегирует субагенту, возвращающему сводку (Isolate). Длина активного окна упала втрое, противоречия исчезли — задачу не «дотянули размером окна», а выправили инженерией контекста.
    ...
    flashcards: [
      ...
      { front: "Гигиена инструментов — правило", back: "Держать <10 инструментов/MCP активными одновременно и <~80 активных в сумме. Схемы tools съедают окно молча: иллюстративно ~200K может схлопнуться до ~70K usable (цифры зависят от объёма схем). Лишние MCP выгружать (lazy-load)." },
      { front: "Хирургическая подача контекста через системный промпт", back: "Вместо свалки всего в CLAUDE.md — точечный --system-prompt под режим (dev/review/research). Грузится только релевантный срез знаний под задачу, а не всё сразу." },
      { front: "Иерархия авторитета контекста", back: "Системный промпт > сообщение пользователя > результат инструмента. Критичные правила режима кладут в системный промпт: они переживают шум из tool-результатов и не перебиваются случайной фразой в чате." }
    ],
    quiz: [
      {
      ...
      {
        question: "Какое практическое правило гигиены контекста по числу активных инструментов?",
        options: [
          "Чем больше MCP подключено, тем лучше",
          "Менее 10 инструментов/MCP одновременно и менее ~80 активных в сумме",
          "Ровно 200 инструментов",
          "Число инструментов не влияет на окно"
        ],
        correct: 1,
        explanation: "Схемы tools занимают окно ещё до первого шага. Правило: <10 активных инструментов/MCP одновременно, <~80 в сумме; лишние MCP выгружать (lazy-load), а не держать «на всякий случай»."
      },
      {
        question: "Как правильно трактовать оценку «окно ~200K схлопывается до ~70K usable»?",
        options: [
          "Это точная фиксированная константа для любой модели",
          "Это иллюстрация: реальная потеря зависит от объёма и многословности схем подключённых инструментов",
          "Это значит, что окно физически уменьшается до 70K навсегда",
          "Это баг конкретной версии CLI"
        ],
        correct: 1,
        explanation: "Цифры 200K→70K — иллюстрация эффекта, а не константа. Конкретная потеря usable-токенов зависит от того, сколько и насколько подробных схем инструментов загружено."
      },
      {
        question: "Какова иерархия авторитета контекста для модели?",
        options: [
          "Результат инструмента > сообщение пользователя > системный промпт",
          "Системный промпт > сообщение пользователя > результат инструмента",
          "Все источники равнозначны",
          "Сообщение пользователя > системный промпт > результат инструмента"
        ],
        correct: 1,
        explanation: "Авторитет убывает: системный промпт > сообщение пользователя > результат инструмента. Поэтому критичные правила режима (dev/review/research) кладут в системный промпт — их не перебьёт шум из tool-результатов."
      }
    ],
    sources: [
